...
## OMP for John
df %>% 
  filter(application == "omp") %>% 
  remove_empty() %>% 
  remove_constant() %>% 
  mutate(
    oai_url = str_extract(oai_url, "[^\n]+"),
    url = str_extract(oai_url, ".+?(?=oai)")
  ) %>% 
  relocate(country, context_name, url, set_spec, version,total_record_count) %>%
  arrange(country, context_name) %>% glimpse()
  #write_csv(here::here("omp_active_for_john.csv"))
```
...
# WoS data
df_wos <-
  read_table(here::here("data/wos_data_original.txt"), na = c("NULL", "NA", "")) %>% 
  clean_names() %>% 
  remove_empty() %>% 
...
# Dimensions data
df_dimensions <-
  read_excel(here::here("data/journals_dimensions.xlsx")) %>%
  clean_names() %>% 
  remove_empty() %>% 
