...
df <-
  ...
  mutate(active = (record_count_from_2019 >= 10)) %>%
...
#   drop_na(country) %>%
#   filter(active) %>%
#   relocate(country, set_spec, context_name, repository_name, domain, issn, record_count_from_2019, total_record_count, active) %>%
#   arrange(country, repository_name, set_spec, context_name, domain, issn, record_count_from_2019, total_record_count) %>%
...
```{r}
df %>% 
  filter(version_clean == "Version 3") %>% 
  drop_na(issn) %>% 
  get_dupes(issn) %>% 
  count(issn)
...
pred_list <- 
  read_csv(here::here("data/predatory/list1.csv")) %>% 
  clean_names() %>% 
  remove_empty()
...
pred_list <-
  bind_cols(
    pred_list,
    pred_list %>% pull(url_publisher) %>% domain() %>% tld_extract()
  ) %>% 
  relocate(domain) %>% 
  mutate(
...
# 43 out of 1133 publisher domains
# 378 journals
# 202 active, 176 inactive
# US and India
df_pred %>% count(domain) %>% arrange(-n)

df_pred %>% count(active)

df_pred %>% filter(active) %>% count(country) %>% arrange(-n)
```
